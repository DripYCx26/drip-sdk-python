...
def mock_charge_result(charge_id: str, is_replay: bool = False) -> dict[str, Any]:
...
class TestComplexIntegration:
    ...
    def test_charge_with_idempotency(self, client: Drip, test_context: TestContext) -> None:
        ...
        respx.post(f"{API_BASE_URL}/charges").mock(
            return_value=httpx.Response(200, json=mock_charge_result(charge_id, is_replay=False))
        )
        ...
        respx.post(f"{API_BASE_URL}/charges").mock(
            return_value=httpx.Response(200, json=mock_charge_result(charge_id, is_replay=True))
        )
        ...
        assert charge2.is_replay is True
    ...
    def test_complete_billing_workflow(self, client: Drip, test_context: TestContext) -> None:
        ...
        # Mock list charges - uses 'data' not 'charges'
